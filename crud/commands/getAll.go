...
func GetAll(w http.ResponseWriter, r *http.Request) {
	...
	sqlQuery := "SELECT command.id, user.id AS id_user, first_name, last_name, command.date, command.state, SUM(command_line.quantity)AS quantity, SUM(command_line.quantity * product.price) FROM command INNER JOIN user ON command.id_user = user.id INNER JOIN command_line ON command_line.id_command = command.id INNER JOIN product ON product.id = command_line.id_product GROUP BY command.id " + orderBy + rangeBy
